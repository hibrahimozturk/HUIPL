    ...
    for j = 1:v
        ...
        for x = 1:m
            for y = 1:n
                ...
                gFilter(x,y) = exp((-((alpha^2)*(xprime^2)+(beta^2)*(yprime^2)))/1)*exp(1i*2*pi*fu*xprime);
            end
        end
...
figure('NumberTitle','Off','Name','Magnitudes of Gabor filters2');
for i = 1:u
    for j = 1:v        
        subplot(u,v,(i-1)*v+j);        
...
figure('NumberTitle','Off','Name','Real parts of Gabor filters2');
for i = 1:u
    for j = 1:v        
        subplot(u,v,(i-1)*v+j);        
